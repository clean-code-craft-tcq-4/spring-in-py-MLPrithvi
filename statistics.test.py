...
class StatsTest(unittest.TestCase):
  ...
  def test_avg_is_nan_for_empty_input(self):
    ...
    self.assertAlmostEqual(m.isnan(computedStats[0]), NAN)
    self.assertAlmostEqual(m.isnan(computedStats[2]), NAN)
    self.assertAlmostEqual(m.isnan(computedStats[1]), NAN)
